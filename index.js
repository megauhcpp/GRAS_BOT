...
const { Client, GatewayIntentBits, ActionRowBuilder, ButtonBuilder, ButtonStyle, EmbedBuilder, SlashCommandBuilder, ChannelType, PermissionsBitField, AttachmentBuilder, StringSelectMenuBuilder, StringSelectMenuOptionBuilder, TextInputBuilder, TextInputStyle, ModalBuilder } = require('discord.js');
...
function getChannelName(username, userId, categoryName) {
    return `tareas-${username.replace(/\./g, '')}-${categoryName.toLowerCase()}-${userId}`;
}
...
async function setupStarterChannel(channel) {
    try {
        ...
        if (existingMessage) {
            ...
            const otherBotMessages = messages.filter(msg => 
                msg.author.id === client.user.id && 
    ...
    try {
        ...
        for (let i = 0; i < STARTER_CHANNELS.length; i++) {
            try {
                ...
                console.error(`Error al configurar el canal ${STARTER_CHANNELS[i]}:`, error);
            }
        }

        // Mostrar mensaje de asignación con todas las categorías
        const categoryNames = userCategories.map(index => ['Calpe', 'Granada', 'Malaga'][index]);
        console.log(`Usuario ${userId} asignado a los starter channels de: ${categoryNames.join(', ')}`);
    } catch (error) {
        console.error(`Error al asignar starter channels para usuario ${userId}:`, error);
    }
}
...
async function updateStarterChannelVisibility(guild, userId) {
    try {
        ...
        console.error(`Error al actualizar visibilidad de starter channels para usuario ${userId}:`, error);
    }
}
...
client.once('ready', async () => {
    ...
    for (const channelId of STARTER_CHANNELS) {
        try {
            ...
            if (channel) {
                ...
                const assignmentChannel = channel.parent.children.cache.find(ch => ch.name === 'asignacion-tareas') ||
    ...
    for (const [memberId, member] of members) {
        if (!member.user.bot) {
            ...
            const hasChannel = guild.channels.cache.some(channel => 
                channel.name === getChannelName(member.user.username, memberId, ['Calpe', 'Granada', 'Malaga'][0])
            );
    ...
    console.log(`Total: ${userRoles.size} usuarios con rol, ${usersWithoutRole.length} usuarios sin rol\n`);
});
    ...
    if (channel.name.startsWith('tareas-')) {
        try {
            ...
            if (member && !member.user.bot) {
                ...
                console.log(`Canal de tareas eliminado para ${member.user.tag}, reasignando starter channel`);
            }
        } catch (error) {
...
client.on('interactionCreate', async (interaction) => {
    ...
    if (interaction.isButton() && interaction.customId === 'create_channel') {
        ...
        if (hasChannelInCategory(interaction.guild, user.username, user.id, categoryName)) {
            await interaction.reply({
        ...
        try {
            ...
            const starterChannelIndex = ['Calpe', 'Granada', 'Malaga'].indexOf(categoryName);
    ...
    if (interaction.isButton() && interaction.customId.startsWith('task_complete_')) {
    ...
    if (interaction.isButton() && interaction.customId.startsWith('confirm_')) {
        ...
        if (status === 'complete' && client.videoQueue?.has(`${interaction.user.id}_${task}`)) {
        ...
        const embed = new EmbedBuilder()
            ...
            .setDescription(`**${interaction.user.username}** ha actualizado el estado de la tarea: "${cleanTask}"`)
            .addFields(
        ...
        if (guild) {
            const channel = guild.channels.cache.find(channel => channel.name === `${interaction.user.username.replace(/\./g, '')}-${interaction.user.id}`);
            if (channel) {
                await channel.send({ embeds: [embed] });
            } else {
                console.log(`Canal no encontrado para el usuario ${interaction.user.username}`);
            }
        } else {
    ...
    if (interaction.isButton() && interaction.customId.startsWith('upload_video_')) {
        ...
        try {
            ...
            const videoInfo = {
                ...
                messageLink: `https://discord.com/channels/${interaction.guild?.id || '@me'}/${originalMessage.channel.id}/${originalMessage.id}`,
    ...
    if (interaction.isButton() && interaction.customId.startsWith('assign_task_')) {
        ...
        if (interaction.channel.name !== 'asignacion-tareas') {
            await interaction.reply({
                content: 'Las tareas solo pueden ser asignadas desde el canal de asignación.',
                flags: [1 << 6]
            });
            return;
        }
    ...
    if (interaction.isModalSubmit() && interaction.customId.startsWith('task_modal_')) {
        ...
        try {
            ...
            const userChannel = interaction.guild.channels.cache.find(channel => {
                const matches = channel.name === channelName && channel.parentId === interaction.channel.parentId;
                console.log(`Comparando canal ${channel.name} (${channel.parentId}) con ${channelName} (${interaction.channel.parentId}): ${matches}`);
                return matches;
            });
            ...
            try {
                const taskWithDescription = description ? `${task}\n\nDescripción:\n${description}` : task;
...
client.on('messageCreate', async (message) => {
    ...
    for (const [userId, user] of mentions) {
        ...
        const userChannel = message.guild.channels.cache.find(ch => {
            const matches = ch.name === channelName && ch.parentId === message.channel.parentId;
            console.log(`Comparando canal ${ch.name} con ${channelName} en categoría ${ch.parent?.name}: ${matches}`);
            return matches;
        });

        if (!userChannel) {
            await message.reply(`No se encontró el canal de tareas para ${user.username} en la categoría ${categoryName}.\nBuscando canal: ${channelName}`);
            continue;
        }
...
client.on('guildMemberUpdate', async (oldMember, newMember) => {
    ...
    if (rolesChanged && !newMember.user.bot) {
        console.log(`Roles actualizados para ${newMember.user.tag}, reconfigurando canales...`);
